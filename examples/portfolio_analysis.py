...
def create_natural_evolution_scenario():
    """Create the natural evolution (empty) scenario if it doesn't exist."""
    scenario_dir = Path("scenarios/natural_evolution")
    scenario_dir.mkdir(parents=True, exist_ok=True)
    
    actions_file = scenario_dir / "actions.csv"
    if not actions_file.exists():
        with open(actions_file, 'w') as f:
            f.write("action_date,action_type,lot_id,quantity,price,notes\n")
            f.write("# Natural Evolution - no actions taken\n")
...
def main():
    ...
    if args.command == 'scenario':
        ...
        # Ensure natural evolution exists
        create_natural_evolution_scenario()
        
        # Create a simple portfolio
        demo_portfolio = Portfolio("Quick Demo", "Compare baseline with exercise strategies")
        demo_portfolio.add_scenario("scenarios/natural_evolution")
        demo_portfolio.add_scenario("scenarios/exercise_all_vested")
        ...
        if scenarios_dir.exists():
            for scenario in sorted(scenarios_dir.iterdir()):
                if scenario.is_dir() and (scenario / "actions.csv").exists():
        ...
        print("\nUsage examples:")
        print("  python portfolio_analysis.py scenario scenarios/exercise_all_vested")
